...
$sql = 'SELECT DISTINCT search_category, vid_category FROM videos WHERE search_category != "" AND vid_category != "newest" AND vid_category NOT LIKE "pro%" ORDER BY search_category ASC';
...
<div class="flex flex-col items-center justify-start fixed h-full bg-black translate-y-16 transition-all duration-500" :class="{ 'w-full': !open, 'w-[calc(100vw-289px)] translate-x-72': open }">
    ...
    <div class="tags-container w-95vw flex flex-row items-center justify-center bg-black text-white rounded-full fixed top-5">
        ...
        <div id="tags" class="tags flex flex-row overflow-x-auto px-3">
            ...
            <!-- Shorts Tag -->
            <div class="tag cursor-pointer whitespace-nowrap flex items-center justify-center bg-gray-800 text-white px-3 mx-1 h-8 w-auto rounded-lg text-sm" data-tag="short-videos">
                Short Videos
            </div>

            <!-- Loop through the array and create a div for each search_category -->
            <?php foreach ($categories as $category) : ?>
                <div class="tag cursor-pointer whitespace-nowrap flex items-center justify-center px-3 mx-1 h-8 w-auto bg-gray-800 text-white rounded-lg text-sm" data-tag="<?php echo $category['vid_category']; ?>">
                    <?php echo $category["search_category"]; ?>
                </div>
            <?php endforeach; ?>
        </div>
    </div>
        ...
        <section>
            ...
            <div class="w-95vw flex flex-col items-center justify-center bg-black text-white px-2 relative">
                ...
                <div id="video-cards" class="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-6 place-items-center gap-2">
                    ...
                    $sql = "SELECT * FROM videos WHERE vid_category != 'newest' AND shorts = 0 ORDER BY $sortColumn $sortOrder LIMIT 12";
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            $shorts = $row['shorts']; // Video Shorts value
                    ...
                    ?>
                            ...
                            <?php if ($shorts != 1) { ?>
                                ...
                                <?php } else { ?>
                                    ...
                                    <div class="video-card aspect-w-16 h-[720px] flex flex-col items-start justify-start overflow-hidden rounded-lg" x-data="{ videoId: '<?php echo $videoId ?>' }">
